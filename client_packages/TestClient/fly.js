...
mp.events.add("render", () => {
  ...
  mp.game.graphics.drawText(
    `Coords: ${coords.x.toFixed(2)}, ${coords.y.toFixed(2)}, ${coords.z.toFixed(
      2
    )} `,
    [0.5, 0.005],
    {
  ...
  mp.game.graphics.drawText(
    `pointAtCoord: ${pointingAt(fly.point_distance).position?.x.toFixed(
      2
    )}, ${pointingAt(fly.point_distance).position?.y.toFixed(2)}, ${pointingAt(
      fly.point_distance
  ...
  if (controls.isControlJustPressed(0, controlsIds.F5)) {
    ...
    mp.game.graphics.notify(
      fly.flying ? "Fly: ~g~Enabled" : "Fly: ~r~Disabled"
    );
  } else if (fly.flying) {
    let updated = false;
    const position = mp.players.local.position;
